...
export const DateData = (props: IDateDataProps) => {
  ...
  const [currentWeekMonday, setCurrentWeekMonday] = useState<Date>(new Date(0));
      ...
      >
        ...
        >
          ...
          <Button
            ...
            className={`${showFirstSeating ? "active" : "disabled"}`}
            type="button"
            border="1px solid white"
            borderRadius="none"
            background="#D9D9D9"
          >
            <Span color="#000000" textAlign="center" fontSizeLaptop="10pt">
    ...
    <Form id="form-3">
      {doneFetching ? (
        <Div>
          ...
          {/* CALENDAR */}
          <Div flexDirectionLaptop="row" justifyContent="center" height="auto">
            {/* SINGLE DAY*/}
            {DAYS_OF_WEEK.map((day, index) => {
              if (currentWeekMonday) {
                return createDay(day, startOfSelectedWeek, index);
              } else return <></>;
            })}
          </Div>
          {/* SELECTED SEATING */}
          ...
          {/* BUTTONS */}
          <Div flexDirection="row" justifyContent="space-between" height="auto">
            {/* BACKWARD BUTTON */}
            <Button
